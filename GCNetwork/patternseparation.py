...
#=========================================================================
# 3. Stimulate randomly 500 cells of 5,000 granule cells 
#=========================================================================
...
def stimulate_GC(iPattern, myNetConList = None):
    ...
    #GC_idx = np.random.choice( range(ecells), 50)
...
def inhibition(pEI, pRI, pLI, debug = None):
    ...
    for pre in range(nRI_PV):
        ...
        for post in RI_GC_idx[ start: end ]:
            netcon.append( PV[pre].connect2target( target = GC[post].isyn, weight=1.235e-5 ))
...
def myrun(pRI = None, pLI = None, netcon = None):
    """
    Executes a simulation by changing the relative inhibitory probabilities

    Example:
    >>> mync = myrun(pRI = 0.24, pLI = 0.3283)
    >>> mync = myrun(pRI = 0.24, pLI = 1, mync)
    """
    
    h.v_init = -80
    if netcon is not None:
        for nt in netcon:
            nt.weight[0] = 0.0 # set connections to zero 
